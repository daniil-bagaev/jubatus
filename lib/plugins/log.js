...
const
    ...
    env = process.env,
    log = require(path.resolve(require(path.resolve('./config/app.config.json')).config.plugins.log.config)),
    logger = {
        env: {
        ...
        log (msg, opts) {
            if (!opts)
                opts = {};
            opts = 
                (!Object.keys(opts).length) ? logger.opts : 
                    (opts.type && logger.types.hasOwnProperty(opts.type)) ? Object.assign(logger.opts, logger.types, opts) : //TODO: побороть типы - не работают
                        {
                            type: 'custom',
